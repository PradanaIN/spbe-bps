...
@section('pages', 'Formulir Usulan Kegiatan')
...
<div class="app-wrapper">
    ...
    <div class="app-content pt-3 p-md-3 p-lg-4">
        <div class="container-xl">
            ...
            <form style="margin: 10px 30px;" action="/usulan-kegiatan" method="POST">
                @csrf
                <input type="hidden" id="satuankerja" name="satuankerja" value="
                @if ($loginby->role_id == 2)
                    {{$loginby->provinsi->nama_provinsi}}
                @elseif ($loginby->role_id == 3)
                    {{$loginby->kabkota->nama_kabkota}}
                @elseif ($loginby->role_id == 0)
                    {{$loginby->nama_user}}
                @endif
                ">
                <div class="mb-3">
                    <label for="nama_usulan" class="form-label">Nama Kegiatan</label>
                    <input type="text" class="form-control @error('nama_usulan') is invalid @enderror" id="nama_usulan" name="nama_usulan" value="{{ old('nama_usulan') }}">
                ...
                <div class="form-group mb-3">
                    <input type="hidden" class="form-control" id="slug_usulan" name="slug_usulan">
                </div>
                ...
                <input type="hidden" name="status_usulan" id="status_usulan" value="0">
                <div class="d-flex flex-row-reverse">
                    <input type="submit" name="submit" value="Tambah Usulan" class="btn btn-primary" style="width: 200px; margin: 20px 0;">
                </div>
            </form>
        </div>
        <!--//container-fluid-->
